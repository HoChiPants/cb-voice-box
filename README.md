...
- Downloads the small English speech model once (about 75 MB) and stores it in your user app-data folder, keeping the application download much smaller.
...
4. Hold PTT, talk, and release. The transcription is copied.
...
The first transcription downloads the tiny English model once. App settings, the model, history, and diagnostics stay in the `CB Voice Box` application-data folder, available through **Open app folder** in the tray menu.
...
- The speech engine is bundled; the model is stored once per computer.
